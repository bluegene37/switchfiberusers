...
// - The job order's own `status` (Scheduled / Completed / Activated) is the
//   only source of stages 2-4. BillingDetails is NOT consulted: its rows are
//   dummy data, and matching on account number once promoted a Completed test
//   job order to Activated using another customer's billing row.
//
// A lookup is therefore two small reads: the Application row, then the job
// order by its public code. jobOrderBelongsTo() checks phone or name before a
// job order is merged into an Application row, so a mislinked row can never
// surface another applicant's schedule.
...
/**
 * Whether a job order is really this applicant's. Guards against a mislinked
 * applicationId: accept the row only when a phone number matches, or both
...
export function createTrackerLookup({ upstream = upstreamJson } = {}) {
  ...
  /**
   * Reconciles an Application row with dispatch (JobOrders).
  ...
  async function enrichApplication(app) {
    ...
    try {
      ...
      applyJobOrder(app, row, { allowCompleted: !isScheduled(current) })
    } catch (err) {
  ...
  async function lookupByRowId(identifier) {
    ...
    return row ? applicationFromJobOrder(row) : null
  }
    ...
    if (res.status === 200 || res.status === 404 || res.status === 400) {
      if (APPLICATION_ROW_ID.test(id)) {
        try {
          ...
          console.warn('[Tracker] legacy number fallback failed:', err?.message || err)
        }
      }
...
// One shared instance per function instance so warm invocations are reused.

...
// Interpolated values land inside the email's HTML, so neutralize markup a
// malicious POST could smuggle into a message that carries our branding.
...
export async function sendConfirmationEmail(data) {
  const clean = (v, max = 150) => escapeHtml(String(v ?? '').trim().slice(0, max))

  const recipientEmail = String(data?.recipientEmail ?? '').trim().slice(0, 100)
  const referenceCode = clean(data?.referenceCode, 40)
  ...
  // The tracking link's origin comes from our own configuration, never from
  // the request body — a crafted POST must not be able to point the applicant
  // at someone else's domain under our branding.
  ...
  const htmlContent = generateEmailHtml({
    applicantName: clean(data?.applicantName, 120),
    referenceCode,
    desiredPlan: clean(data?.desiredPlan, 100),
    installationAddress: clean(data?.installationAddress, 255),
    barangay: clean(data?.barangay, 100),
    city: clean(data?.city, 100),
    mobileNumber: clean(data?.mobileNumber, 20),
    firstNearestLandmark: clean(data?.firstNearestLandmark, 255),
    trackingUrl
  })
  ...
  if (!response.ok) {
    // Common on the free tier: onboarding@resend.dev may only send to the
  ...
  console.log(`[Resend Service] Email dispatched to ${recipientEmail} (ID: ${resData?.id})`)
  return { success: true, id: resData?.id, recipientEmail, referenceCode }
}

// Vercel Serverless Function Handler.
// Same-origin only — no CORS grant, same policy as api/_proxy.js. An open
// header here would let any website spend our Resend quota.
